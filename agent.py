...
TOOLS = [
    ...
    {"name": "get_subscribed_channels", "description": "Googleアカウントの購読YouTubeチャンネル一覧を取得", "input_schema": {"type": "object", "properties": {}, "required": []}},
    {"name": "get_youtube_summary_videos", "description": "購読チャンネルのN時間以内の新着動画を取得し字幕要約付きで返す", "input_schema": {"type": "object", "properties": {"hours": {"type": "integer", "default": 24}}, "required": []}},
    {"name": "get_google_tasks_due_soon", "description": "全リスト横断で期限N日以内のタスクを返す", "input_schema": {"type": "object", "properties": {"days": {"type": "integer", "default": 3}}, "required": []}},
    {"name": "get_upcoming_tasks", "description": "全Googleタスクリストを横断して期限N日以内のタスクを期限順で返す", "input_schema": {"type": "object", "properties": {"due_within_days": {"type": "integer", "default": 3}}, "required": []}},
    {"name": "get_google_task_lists", "description": "Googleタスクのリスト一覧を取得する", "input_schema": {"type": "object", "properties": {}, "required": []}},
    {"name": "get_google_tasks", "description": "Googleタスクのリストからタスクを取得。due_within_days指定で期限N日以内のみ返す", "input_schema": {"type": "object", "properties": {"tasklist_title": {"type": "string", "default": "バケツリスト"}, "show_completed": {"type": "boolean", "default": False}, "due_within_days": {"type": "integer", "description": "期限N日以内のタスクのみ返す（省略時は全件）"}}, "required": []}},
